...
class Embed(Embed):
    ...
    def set_author(
        self, name: str = "", url: str = UNSET, icon_url: str = UNSET, proxy_icon_url: str = UNSET
    ) -> "Embed":
...
class Channel(Channel):
    async def get_messages(
        self, before_id: Snowflake = None, messages: list[Message] = [], limit: int = 100
    ) -> list["Message"]:
        if limit < 1:
            return messages
